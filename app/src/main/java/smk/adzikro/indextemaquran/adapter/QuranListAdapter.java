...
import smk.adzikro.indextemaquran.object.Tag;
import smk.adzikro.indextemaquran.ui.TagsViewGroup;
import smk.adzikro.indextemaquran.object.Bookmark;
import smk.adzikro.indextemaquran.ui.JuzView;
...
public class QuranListAdapter extends
  ...
  private static final String TAG ="QuranListAdapter" ;
  ...
  private void bindRow(HeaderHolder vh, int position) {
    ...
    String[] listAyat=null;
   // Log.e(TAG,"Jumlah ayat "+item.sura);
    holder.noSurat.setText(QuranUtils.getLocalizedNumber(mContext, item.sura));
    int countAyat = BaseQuranData.SURA_NUM_AYAHS[item.sura-1];
    int l;
    listAyat = new String[countAyat];
    for (l = 0; l < countAyat; l++) {
      listAyat[l] = String.valueOf(l + 1);
    }
    ArrayAdapter<String> list_ayat = new ArrayAdapter<String>(mContext, R.layout.header_nama_surat, listAyat);
    list_ayat.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
    holder.number.setAdapter(list_ayat);
  ...
  public void onClick(View v) {
    ...
    if (position != RecyclerView.NO_POSITION) {
      ...
      Spinner spinner = v.findViewById(R.id.pageNumber);
      int x =spinner.getSelectedItemPosition()+1;
      int page;
      if(x<=0)
        page = element.page;
      else
        page = BaseQuranInfo.getPageFromSuraAyah(element.sura,x);
      if (mTouchListener == null) {
        ((MainActivity) mContext).jumpTo(page,-1);
        Toast.makeText(mContext, "Halaman " + element.page, Toast.LENGTH_SHORT).show();
      } else {
  ...
  class ViewHolder extends HeaderHolder {
    ...
    public ViewHolder(View itemView) {
      ...
      number = itemView.findViewById(R.id.pageNumber);
      noSurat = itemView.findViewById(R.id.suraNumber);
