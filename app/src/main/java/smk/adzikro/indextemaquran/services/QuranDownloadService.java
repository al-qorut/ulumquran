...
import smk.adzikro.indextemaquran.services.utils.QuranDownloadNotifier.ProgressIntent;
import smk.adzikro.indextemaquran.services.utils.QuranDownloadNotifier.NotificationDetails;
...
import smk.adzikro.indextemaquran.util.ZipUtils;
...
public class QuranDownloadService extends Service implements
  ...
  public static final String ACTION_DOWNLOAD_URL =
      "smk.adzikro.indextemaquran.DOWNLOAD_URL";
  public static final String ACTION_CANCEL_DOWNLOADS =
      "smk.adzikro.indextemaquran.CANCEL_DOWNLOADS";
  public static final String ACTION_RECONNECT =
      "smk.adzikro.indextemaquran.RECONNECT";
...
//    ((QuranApplication) getApplication()).getApplicationComponent().inject(this);
 ...
 //   Log.e(TAG,"handleOnStartCommand "+intent.getExtras().getString(QuranDownloadService.EXTRA_URL));
  ...
  private boolean downloadRange(String urlString,
    ...
    if (isGapless) {
      ...
      if (startSura == endSura) {
        ...
        for (int i = startSura + 1; i < endSura; i++) {
          totalAyahs += BaseQuranInfo.getNumAyahs(i);
        }

        // add the number of ayahs from the start sura
        totalAyahs += BaseQuranInfo.getNumAyahs(startSura) - startAyah + 1;
    ...
    Log.d(TAG, String.format("downloadRange for %d between %d:%d to %d:%d, gaplessFlag: %s",
  ...
  private int downloadUrl(String url, String path, String filename,
    ...
    //    Crashlytics.log("successful response: " + response.code() + " - " + downloadedAmount);
      ...
      Log.e(TAG, "Security exception while downloading file");
    } finally {
