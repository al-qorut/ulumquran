...
public class AyahToolBar extends ViewGroup implements
  ...
  private void init(Context context) {
    ...
    final int background = ContextCompat.getColor(context, R.color.tranparent);
  ...
  private View getMenuItemView(MenuItem item) {
    ...
    button.setImageDrawable(getGambarIcon(item.getIcon()));
    //button.setImageBitmap(getGambarIcon(item.getIcon()));
  ...
  @NonNull
  private Drawable getGambarIcon(Drawable img){
    Bitmap bitmap = ((BitmapDrawable)img).getBitmap();
    float radius = bitmap.getWidth()*0.8f;
    Bitmap canvasBitmap = Bitmap.createBitmap(bitmap.getWidth()*2,
    ...
    canvas.drawCircle(canvasBitmap.getWidth() / 2, canvasBitmap.getHeight() / 2,
            radius, paint);
    canvas.drawBitmap(bitmap,(canvasBitmap.getWidth()-bitmap.getWidth())/2,(canvasBitmap.getHeight()-bitmap.getHeight())/2,paint);
